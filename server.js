...
app.put('/api/products/:id/timeline', (req, res) => {
    const { stage, location, date, temperature, status } = req.body;
    
    const data = readData();
    const productIndex = data.products.findIndex(p => p.id === req.params.id.toUpperCase());
    ...
    // Update or add timeline entry
    const timelineIndex = product.timeline.findIndex(t => t.stage === stage);
    if (timelineIndex !== -1) {
        product.timeline[timelineIndex] = { stage, location, date, temperature, status };
    } else {
        product.timeline.push({ stage, location, date, temperature, status });
    }
